...
const Content = () => {
  ...
  const partNames = useMemo(() => {
    if (selectedCategory === "Cylinder Head") {
      return ["Cylinder Head (Complete)", "Cylinder Head (Partial)"];
    }
    return [...new Set(allProducts.map((p) => p.partname))];
  }, [selectedCategory, allProducts]);

  const companies = useMemo(() => {
    if (selectedCategory === "Cylinder Head") {
      return [...new Set(allProducts
        .filter(p => ["Cylinder Head (Complete)", "Cylinder Head (Partial)"].includes(p.partname))
        .map(p => p.make)
      )];
    }
    return [...new Set(allProducts.map(p => p.make))];
  }, [selectedCategory, allProducts]);

  const models = useMemo(() => {
    return [...new Set(allProducts.map((p) => p.Model))];
  }, [allProducts]);

  const seriesNames = useMemo(() => {
    return [...new Set(allProducts.map((p) => p.seriesName).filter(Boolean))];
  }, [allProducts]);
  ...
  const filteredProducts = useMemo(() => {
    return allProducts.filter((product) => {
      ...
      const matchModel = filterModel ? product.Model === filterModel : true;
      const matchSeries = filterSeries ? product.seriesName === filterSeries : true;
      return matchPart && matchMake && matchModel && matchSeries;
    });
  }, [allProducts, filterPartName, filterMake, filterModel, filterSeries]);
...
//                     <p><span>Make:</span> {product.Make}</p>
//                     <p><span>Model:</span> {product.Model}</p>
//                     <p><span>Products:</span> {product.products}</p>
//                   </div>
//                   <button
//                     className="view-button"
//                     onClick={() => navigate(`/product/${product.id}`)}
